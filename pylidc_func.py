...
def ann_group(suid, hu_a):
    ...
    for i, ann_cluster in enumerate(s.cluster_annotations()):
        ...
        for ann in ann_cluster:
            ...
            upper_row.append(bbox[0][1])
            lower_row.append(bbox[1][1])
            upper_col.append(bbox[0][2])
            lower_col.append(bbox[1][2])
            diameters.append(ann.diameter)
        # print("upper: ", upper_ndx)
        # print("lower: ", lower_ndx)
        ann_data.append((suid, i, min(upper_ndx), max(lower_ndx), min(upper_row), max(lower_row), min(upper_col), max(lower_col), max(diameters)))
    return ann_data
