...
def dc(result, reference):
    ...
    ----------
    result : torch.Tensor
        Input data containing objects. Should be a binary tensor: background as 0, object as 1.
    ...
    intersection = torch.count_nonzero(result & reference).item()

    size_i1 = torch.count_nonzero(result).item()
    size_i2 = torch.count_nonzero(reference).item()
...
def intersection_percentage(result, reference):
    result = torch.as_tensor(result, dtype=torch.bool)
    reference = torch.as_tensor(reference, dtype=torch.bool)

    intersection = torch.count_nonzero(result & reference).item()
    label_count = torch.count_nonzero(reference).item()
    
    # Calculate the percentage of the intersection area
    intersection_percentage = float(intersection) / float(label_count)
...
def jc(result, reference):
    ...
    intersection = torch.count_nonzero(result & reference).item()
    union = torch.count_nonzero(result | reference).item()
...
def precision(result, reference):
    ...
    tp = torch.count_nonzero(result & reference).item()
    fp = torch.count_nonzero(result & ~reference).item()
...
def recall(result, reference):
    ...
    tp = torch.count_nonzero(result & reference).item()
    fn = torch.count_nonzero(~result & reference).item()
...
def specificity(result, reference):
    ...
    Notes
    -----
    Not symmetric. The complement of the specificity is :func:`sensitivity`.
    ...
    tn = torch.count_nonzero(~result & ~reference).item()
    fp = torch.count_nonzero(result & ~reference).item()
...
def assd(result, reference, voxelspacing=None, connectivity=1):
    ...
    >>> assd(result, reference)

    and

    >>> assd(reference, result)
